...
def generate_new_refresh_token():
    try:
        auth_code = read_json_from_file("token.json").get("code")
        payload = {
            "client_id": CLIENT_ID,
...
# Write the data to token.json
    ...
    try:
        ...
        access_token = read_json_from_file("token.json").get("access_token")
        headers = {
        ...
        if response.status_code == 401:
            ...
            refresh_token = read_json_from_file("token.json").get("refresh_token")
            payload = {
                "client_id": CLIENT_ID,
                "client_secret": CLIENT_SECRET,
